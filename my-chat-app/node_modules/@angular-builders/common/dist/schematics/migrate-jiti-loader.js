...
exports.migrateToJitiLoader = migrateToJitiLoader;
 ...
 */
function migrateToJitiLoader(packageName) {
    return async (tree, context) => {
        ...
        return (0, schematics_1.chain)([
            ...
            (0, rules_1.removeDevDependencies)(['ts-node', 'tsconfig-paths']),
            migrateTsNodeTsconfigSections(packageName, tsconfigPaths),
            adviseTypeChecking(packageName),
        ]);
    };
}
function collectTsconfigPaths(workspace) {
    const paths = new Set(['/tsconfig.json', '/tsconfig.base.json']);
...
function stripTsNodeEsmFromScripts(packageName) {
    return (tree, context) => {
        if (!tree.exists('/package.json'))
            return tree;
        const json = new json_file_1.JSONFile(tree, '/package.json');
        const scripts = json.get(['scripts']);
        if (!scripts)
            return tree;
        let changed = false;
        for (const [name, value] of Object.entries(scripts)) {
            ...
            const next = value
                ...
                .replace(/\bNODE_OPTIONS=(?:'--loader ts-node\/esm'|"--loader ts-node\/esm"|--loader[= ]ts-node\/esm)\s+/g, '')
        ...
        if (changed) {
            context.logger.info(`[${packageName}] Removed the \`ts-node/esm\` NODE_OPTIONS loader workaround from npm ` +
                `scripts — TypeScript configs now load via jiti with plain \`ng\` commands.`);
        }
        return tree;
    };
}
function migrateTsNodeTsconfigSections(packageName, tsconfigPaths) {
    return (tree, context) => {
        for (const path of tsconfigPaths) {
            if (!tree.exists(path))
                continue;
            const json = new json_file_1.JSONFile(tree, path);
            const section = json.get(['ts-node']);
            const tsNodeCompilerOptions = section?.compilerOptions;
            if (!tsNodeCompilerOptions)
                continue;
            ...
            if (paths && typeof paths === 'object') {
                const existing = json.get(['compilerOptions', 'paths']) ?? {};
            ...
            if (leftover.length > 0) {
                ...
                context.logger.warn(`[${packageName}] ${path}: the \`ts-node\` section is no longer read by the builder ` +
                ...
                json.remove(['ts-node']);
                context.logger.info(`[${packageName}] ${path}: migrated the \`ts-node\` section's path options into ` +
...
function ensureCompilerOptions(json) {
...
function adviseTypeChecking(packageName) {
    return (tree, context) => {
        context.logger.warn(`[${packageName}] TypeScript configs/plugins are now transpiled by jiti and are NO LONGER ` +
...
//# sourceMappingURL=migrate-jiti-loader.js.map