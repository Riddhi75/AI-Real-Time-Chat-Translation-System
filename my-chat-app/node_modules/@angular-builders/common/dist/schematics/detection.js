...
exports.getProjectsToTarget = getProjectsToTarget;
exports.detectTestBuilder = detectTestBuilder;
exports.isZoneless = isZoneless;
function getProjectsToTarget(workspace, optionProject) {
...
function detectTestBuilder(workspace, projectName) {
    ...
    if (builder.endsWith(':karma'))
        return 'karma';
    if (builder === '@angular-builders/jest:run')
        return 'jest';
...
function isZoneless(tree, workspace, projectName) {
    const project = workspace.projects.get(projectName);
    ...
    const mainCandidates = ['src/main.ts', 'src/app/app.config.ts'].map((p) => root ? `${root}/${p}` : p);
