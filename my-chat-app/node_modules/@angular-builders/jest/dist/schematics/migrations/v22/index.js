...
exports.migrateV22 = migrateV22;
const utility_1 = require("@schematics/angular/utility");
function migrateV22() {
    return async (tree, context) => {
        context.logger.warn('[@angular-builders/jest] v22: ts-jest `isolatedModules` now defaults to true. ' +
        ...
        context.logger.warn('[@angular-builders/jest] v22: a TypeScript jest config (e.g. `jest.config.ts`) is now ' +
        ...
        tree.visit(path => {
            ...
            if (/\bconst\s+enum\b/.test(content))
                constEnumHits.push(path);
        });
        if (constEnumHits.length > 0) {
            context.logger.warn('[@angular-builders/jest] Found `const enum` in: ' +
                constEnumHits.join(', ') +
        ...
        if (affected.length > 0) {
            context.logger.warn('[@angular-builders/jest] v22: per-project coverage output now writes to ' +
